...
3. `再取得` でspinnerが出ることを確認する。
4. 申請番号をクリックし、詳細モーダルが開くことを確認する。
5. 新規申請を1件登録する。
6. Toast、一覧更新、詳細表示、Adminer上のDB登録値を確認する。
...
- 主催者専用: `docs/facilitator` と `facilitator-assets`

参加者向け配布物に、主催者向け模範解答、テストコード、mutation候補を含めないでください。
...
網羅率は「どのコードを通したか」を見るための指標です。品質判断では、入力値、条件分岐、エラー処理、DB登録値、通信経路、運用手順の妥当性もあわせて確認してください。
